...
CREATE INDEX IF NOT EXISTS idx_slack_messages_title_number ON slack_messages(title_number);
CREATE INDEX IF NOT EXISTS idx_slack_messages_category ON slack_messages(category);
CREATE INDEX IF NOT EXISTS idx_slack_messages_is_reply ON slack_messages(is_reply);
CREATE INDEX IF NOT EXISTS idx_slack_messages_title_name_trgm
  ON slack_messages USING gin (title_name gin_trgm_ops);  -- 작품명 부분검색

-- pg_trgm extension (부분검색용)
