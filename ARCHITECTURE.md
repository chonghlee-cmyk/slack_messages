...
flowchart LR
    ...
    D --> E["📈 웹 대시보드<br/>(toomics-dashboard.vercel.app)"]
...
```mermaid
flowchart LR
    A["Google Sheets<br/>(완성된 데이터)"] --> B["Supabase 테이블에<br/>업로드 (upsert)"]
    B --> C["대시보드에서<br/>빠른 검색 가능"]
```
...
```mermaid
flowchart LR
    A["작품 목록<br/>(검색/필터)"] --> B["작품 상세"]
    B --> C["언어권별 정보<br/>(PT/EN/ES/...)"]
    B --> D["원고 수정사항"]
    B --> E["Slack 메시지<br/>(카테고리 그룹)"]
    B --> F["언어권별 메모"]
```
...
- [x] ~~AI 분류 마무리~~ (진행 중)
- [x] ~~대시보드 만들기~~ → **완료** (https://toomics-dashboard.vercel.app)
- [ ] 작품별 이슈 트렌드 시각화
- [ ] 자동 알림 (특정 이슈 발생 시)
- [ ] 슬랙 채널 추가 (현재 1개 → 다수)
